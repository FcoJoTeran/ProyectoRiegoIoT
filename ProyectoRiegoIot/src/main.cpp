...
void actualizarLecturasTask(void *pvParameters) {
  ...
  while (true) {
    ...
    actualizarLecturas();
    enviarDatosAlServidor(); // Actualiza los datos en el servidor remoto Agregado hoy 13 agosto
...
void controlarRiegoTask(void *pvParameters) {
  ...
  while (true) {
    ...
    vTaskDelay(pdMS_TO_TICKS(5000)); // Espera 5 segundos entre controles de riego
...
void mostrarEnLCDTask(void *pvParameters) {
  ...
  while (true) {
    ...
    mostrarEnLCD();
    vTaskDelay(pdMS_TO_TICKS(3000)); // Espera 5 segundos entre actualizaciones del LCD
  }
}
